...
<xsl:template name="str:minus-tokens">
  <xsl:param name="tokens"/>
        ...
        <xsl:variable name="out">
          <xsl:choose>
            <xsl:when test="contains($padded, $minus-first)">
              ...
              <xsl:text> </xsl:text>
              <xsl:call-template name="str:minus-tokens">
...
<xsl:template match="html:*" mode="add-meta-meta">
  ...
  <xsl:variable name="first">
    <xsl:call-template name="str:safe-first-token">
      <xsl:with-param name="token" select="$targets"/>
    </xsl:call-template>
  </xsl:variable>
...
<xsl:template match="html:*" mode="rdfa:filter-by-type">
  ...
  <xsl:variable name="first">
    <xsl:call-template name="str:safe-first-token">
      <xsl:with-param name="token" select="$subjects"/>
    </xsl:call-template>
  </xsl:variable>
...
<xsl:template match="html:*" mode="ibis:datalist-options">
  ...
  <xsl:variable name="first">
    <xsl:call-template name="str:safe-first-token">
      <xsl:with-param name="token" select="$inventories"/>
    </xsl:call-template>
  </xsl:variable>
...
<xsl:template match="html:*" mode="ibis:actual-option">
  ...
  <xsl:variable name="first">
    <xsl:call-template name="str:safe-first-token">
      <xsl:with-param name="token" select="$resources"/>
    </xsl:call-template>
  </xsl:variable>
...
<xsl:template match="*" mode="link-stack">
  ...
  <xsl:if test="$s">
    <xsl:variable name="first">
      <xsl:call-template name="str:safe-first-token">
        <xsl:with-param name="token" select="$s"/>
      </xsl:call-template>
    </xsl:variable>
...
<xsl:template match="html:*" mode="cgto:select-focus">
  ...
  <section>
    ...
    <form>
      <select>
        <option>Issue Network</option>
        <option>Concept Scheme</option>
      </select>
      <input type="text"/>
      <button></button>
    </form>
